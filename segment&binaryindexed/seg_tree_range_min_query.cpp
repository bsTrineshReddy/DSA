...
void update(int *st, int si, int ss, int se, int pos, int newVal) {
        if (ss == se && se==pos) {
            ...
            if (pos <= tm) {
                ...
                update(st, 2 * si+1, ss, mid, pos, newVal);
            } else {
                // Update in the right subtree
                update(2 * si + 2, mid + 1, se, pos, newVal);
            }
            // Update the current node's value after updating the child nodes
